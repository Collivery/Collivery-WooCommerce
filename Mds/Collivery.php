...
class Collivery {
	...
	public function searchTowns( $name )
	{
		if ( strlen( $name ) < 2 ) {
			...
			if ( isset( $result ) ) {
				if ( $this->check_cache != 0 ) $this->cache->put( 'collivery.search_towns.'. $name, $result, 60*24 );
	...
	public function getLocationTypes()
	{
		if ( ( $this->check_cache == 2 ) && $this->cache->has( 'collivery.location_types' ) ) {
			...
			if ( isset( $result['results'] ) ) {
				if ( $this->check_cache != 0 ) $this->cache->put( 'collivery.location_types', $result['results'], 60*24*7 );
	...
	public function getServices()
	{
		if ( ( $this->check_cache == 2 ) && $this->cache->has( 'collivery.services' ) ) {
			...
			if ( isset( $result['services'] ) ) {
				if ( $this->check_cache != 0 ) $this->cache->put( 'collivery.services', $result['services'], 60*24*7 );
	...
	public function getParcelTypes()
	{
		if ( ( $this->check_cache == 2 ) && $this->cache->has( 'collivery.parcel_types' ) ) {
			...
			if ( is_array( $result ) ) {
				if ( $this->check_cache != 0 ) $this->cache->put( 'collivery.parcel_types', $result, 60*24*7 );
				return $result;
			} else {
				if ( isset( $result['error_id'] ) )
					$this->setError( $result['error_id'], $result['error'] );
	...
	public function getAddress( $address_id )
	{
		if ( ( $this->check_cache == 2 ) && $this->cache->has( 'collivery.address.'. $this->client_id .'.'. $address_id ) ) {
			...
			if ( isset( $result['address'] ) ) {
				if ( $this->check_cache != 0 ) $this->cache->put( 'collivery.address.'. $this->client_id .'.'. $address_id, $result['address'], 60*24 );
	...
	public function getContacts( $address_id )
	{
		if ( ( $this->check_cache == 2 ) && $this->cache->has( 'collivery.contacts.'. $this->client_id .'.'. $address_id ) ) {
			...
			if ( isset( $result['contacts'] ) ) {
				if ( $this->check_cache != 0 ) $this->cache->put( 'collivery.contacts.'. $this->client_id .'.'. $address_id, $result['contacts'], 60*24 );
