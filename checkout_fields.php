...
function custom_override_default_fields($fields) {
    ...
    $data = Array(
        ...
        'suburb' => Array(
            ...
            'options' => array('---Please select a town first---'),
            'selected' => ''
        ),
        'location_type' => Array(
            'type' => 'select',
...
function generate_suburbs() {
    // Here so we can auto select the clients suburb
    if (WC()->session->get_customer_id() > 0 && $_POST['type']) {
        global $wpdb;
        $table_name = $wpdb->prefix . 'usermeta';
        $config = $wpdb->get_results("SELECT * FROM `" . $table_name . "` WHERE user_id=" . WC()->session->get_customer_id() . " and meta_key='" . $_POST['type'] . "suburb';", OBJECT);
